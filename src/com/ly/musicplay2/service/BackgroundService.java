	...
	class MusicController extends Binder implements MusicInterface {
		...
		public void next() {
			...
			Log.d("BackgroundService", "正在播放的MusicActivity.songNum "
					+ SDPager.songNum);
			if (mediaPlayer != null) {
				if (SDPager.musicList != null) {
					if (SDPager.songNum == (SDPager.musicList.size() - 1)) {
						SDPager.songNum = 0;
					} else {
					...
					Log.d("BackgroundService", "判断完后播放的SDPager.songNum "
							+ SDPager.songNum);
					songName = SDPager.musicList.get(SDPager.songNum);
			...
			if (mediaPlayer != null) {
				if (SDPager.musicList != null) {
					if (SDPager.songNum == 0) {
						SDPager.songNum = (SDPager.musicList.size() - 1);
					} else {
						SDPager.songNum = SDPager.songNum - 1;
					}
					Log.d("BackgroundService", "判断完后播放的SDPager.songNum "
